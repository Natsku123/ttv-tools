...
import {deleteEventSub, getEventSub} from "@/services/eventsubs";
...
export default function EventsubPage() {
    ...
    const queries: Array<QueriesOptions<any>> = [
        {
            queryKey: ["eventsubs", router.query.uuid],
            queryFn: () => getEventSub(router.query.uuid),
            options: {
                retry: false
            }
        },
            ...
            { isLoading ? <>
                ...
                { data && discordServer && discordChannel && <>
                    <Grid container spacing={4} alignItems={"center"}>
                        ...
                        <Grid item xs>
                            <Button variant={"outlined"} color={"error"} onClick={() => remove.mutate(router.query.uuid as string)}><DeleteIcon /></Button>
                        </Grid>
                    </Grid>
                </>}
            </> }
